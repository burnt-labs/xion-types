...
def main():
    ...
    # Remove types directory to prevent SPM from scanning raw generated files
    # (they've already been copied into Sources/ targets above)
    if Path("swift/types").exists():
        print("Removing swift/types/ (files already copied to Sources/)...")
        shutil.rmtree("swift/types")

    # Clean up stale temp directory from previous runs
    if Path("swift-types-temp").exists():
        shutil.rmtree("swift-types-temp")
