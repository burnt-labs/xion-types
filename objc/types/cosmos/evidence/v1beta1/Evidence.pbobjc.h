// Generated by the protocol buffer compiler.  DO NOT EDIT!
// NO CHECKED-IN PROTOBUF GENCODE
// clang-format off
// source: cosmos/evidence/v1beta1/evidence.proto

// This CPP symbol can be defined to use imports that match up to the framework
// imports needed when using CocoaPods.
#if !defined(GPB_USE_PROTOBUF_FRAMEWORK_IMPORTS)
 #define GPB_USE_PROTOBUF_FRAMEWORK_IMPORTS 0
#endif

#if GPB_USE_PROTOBUF_FRAMEWORK_IMPORTS
 #import <Protobuf/GPBProtocolBuffers.h>
#else
 #import "GPBProtocolBuffers.h"
#endif

#if GOOGLE_PROTOBUF_OBJC_VERSION < 30007
#error This file was generated by a newer version of protoc which is incompatible with your Protocol Buffer library sources.
#endif
#if 30007 < GOOGLE_PROTOBUF_OBJC_MIN_SUPPORTED_VERSION
#error This file was generated by an older version of protoc which is incompatible with your Protocol Buffer library sources.
#endif

// @@protoc_insertion_point(imports)

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"

CF_EXTERN_C_BEGIN

NS_ASSUME_NONNULL_BEGIN

#pragma mark - EvidenceRoot

/**
 * Exposes the extension registry for this file.
 *
 * The base class provides:
 * @code
 *   + (GPBExtensionRegistry *)extensionRegistry;
 * @endcode
 * which is a @c GPBExtensionRegistry that includes all the extensions defined by
 * this file and all files that it depends on.
 **/
GPB_FINAL @interface EvidenceRoot : GPBRootObject
@end

#pragma mark - Equivocation

typedef GPB_ENUM(Equivocation_FieldNumber) {
  Equivocation_FieldNumber_Height = 1,
  Equivocation_FieldNumber_Time = 2,
  Equivocation_FieldNumber_Power = 3,
  Equivocation_FieldNumber_ConsensusAddress = 4,
};

/**
 * Equivocation implements the Evidence interface and defines evidence of double
 * signing misbehavior.
 **/
GPB_FINAL @interface Equivocation : GPBMessage

/** height is the equivocation height. */
@property(nonatomic, readwrite) int64_t height;

/** time is the equivocation time. */
@property(nonatomic, readwrite, strong, null_resettable) GPBTimestamp *time;
/** Test to see if @c time has been set. */
@property(nonatomic, readwrite) BOOL hasTime;

/** power is the equivocation validator power. */
@property(nonatomic, readwrite) int64_t power;

/** consensus_address is the equivocation validator consensus address. */
@property(nonatomic, readwrite, copy, null_resettable) NSString *consensusAddress;

@end

NS_ASSUME_NONNULL_END

CF_EXTERN_C_END

#pragma clang diagnostic pop

// @@protoc_insertion_point(global_scope)

// clang-format on
